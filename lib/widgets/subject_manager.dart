...
class SubjectManager extends StatefulWidget {
  ...
  const SubjectManager({super.key, this.onSubjectsChanged});
...
class _SubjectManagerState extends State<SubjectManager> {
  ...
  void _addSubject() async {
    ...
    if (newSubjectName.isNotEmpty &&
        !_subjects.any((s) => s.name == newSubjectName)) {
      final newSubject = Subject(uuid: const Uuid().v4(), name: newSubjectName);
  ...
  void _saveEditSubject() async {
    ...
    if (newName.isNotEmpty && _editingSubjectUuid != null) {
      final subject = _subjects.firstWhere(
        (s) => s.uuid == _editingSubjectUuid,
      );
      final updatedSubject = Subject(uuid: subject.uuid, name: newName);
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Icon(Icons.subject, color: colorScheme.primary, size: 24),
