...
class TagManager extends StatefulWidget {
  ...
  const TagManager({super.key, this.onTagsChanged});
...
class _TagManagerState extends State<TagManager> {
  ...
  void _addTag() async {
    ...
    if (newTagName.isNotEmpty && !_tags.any((t) => t.name == newTagName)) {
      final newTag = Tag(uuid: const Uuid().v4(), name: newTagName);
  ...
  void _saveEditTag() async {
    ...
    if (newName.isNotEmpty && _editingTagUuid != null) {
      ...
      final updatedTag = Tag(uuid: tag.uuid, name: newName);
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Icon(Icons.label, color: colorScheme.primary, size: 24),
